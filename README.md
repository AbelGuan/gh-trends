...
──────
🔥 今日涨星                                  ← 两榜合并成两行
**最多** owner/repo +3,606 · owner/repo +3,290 · owner/repo +1,350
**最猛** owner/repo +52.9% · owner/repo +49.9% · owner/repo +23.7%
──────
...
- **「🔥 今日涨星」一行一个维度**：「最多」看绝对增量（大仓库占优），「最猛」看相对增速（小仓库容易露头）。
  「最猛」的分母用 `stars - today`（昨日星数）而不是当前总星，否则今天的增长会把自己的分母撞大。
  默认过滤 `昨日星数 < 500` 或 `今日新增 < 50` 的项目，避免小数仓/微小波动制造假异常
  （阈值在 `star_board(min_stars=, min_today=)`）。`--hot N` / `--odd N` 分别控制每行几名（0 = 不显示那行）。
- **名次变化每类最多列 3 个**，超出时显示“（共 N 个）”，避免新上榜太多时那一行超长。
